...
function salvaStatoTabella() { // TABELLA INIZIALE
	...
	localStorage.setItem("tabellaHTMLCOM", tableHTML);
}

function ripristinaTabella() {
	var table = document.getElementById("myTable");
	var tableHTML = localStorage.getItem("tabellaHTMLCOM");
	if (tableHTML) {
...
function salvaStatoIndici() { // TABELLA INDICI
    ...
    localStorage.setItem("indiciHTMLCOM", indexHTML);
...
function ripristinaIndici() {
    ...
    var indexHTML = localStorage.getItem("indiciHTMLCOM");
    if (indexHTML) {
		var indexContainer = document.querySelector(".index-container");
...
function aggiungiParagrafo(chapterID) {
	...
	columnDiv.appendChild(tornaInizio); //aggiunto tasto modifica codice
...
function salvaStrutturaLocalStorage() {
    ...
    localStorage.setItem('strutturaPaginaProvaCOM', strutturaPagina);
...
function ripristinaStrutturaLocalStorage() {
    ...
    var strutturaPagina = localStorage.getItem('strutturaPaginaProvaCOM');
...
function exportToExcel() {
    ...
    // Scrivere il file
    XLSX.writeFile(wb, 'estrazione_dati.xlsx');
}
...
function importFromExcel() {
    ...
    reader.onload = function(event) {
        ...
        // Ricrea l'HTML dalla struttura del file
        let chapterHTML = '';
        json.forEach((row, index) => {
            if (row.length === 1 && row[0].startsWith('CAP')) {
                if (index > 0) {
                    chapterHTML += '</div>';
                    content.innerHTML += chapterHTML;
                }
                chapterHTML = `<div class="chapter-container"><div class="chapter-title">${row[0]}</div>`;
            } else if (row.length === 1 && row[0].startsWith('PAR')) {
                chapterHTML += `<div class="section"><div class="section-column1"><div class="section-title">${row[0]}</div><hr>`;
            } else if (row.length === 1 && row[0].trim().startsWith('SELECT')) {
                chapterHTML += `<pre class="bkg-section"><code class="language-sql">${row[0]}</code></pre></div>`;
            } else if (row.length > 1) {
                chapterHTML += `<div class="section-column2"><table><thead><tr>`;
                row.forEach(cell => {
                    chapterHTML += `<th>${cell}</th>`;
                });
                chapterHTML += `</tr></thead><tbody>`;
                json.slice(index + 1).forEach(innerRow => {
                    if (innerRow.length === row.length) {
                        chapterHTML += `<tr>`;
                        innerRow.forEach(cell => {
                            chapterHTML += `<td>${cell}</td>`;
                        });
                        chapterHTML += `</tr>`;
                    }
                });
                chapterHTML += `</tbody></table></div>`;
            }
        });
